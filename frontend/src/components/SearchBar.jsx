import React, { useState, useRef, useEffect } from "react";
import { Search, X, ChevronDown, Filter } from "lucide-react";

const SearchBar = ({
  searchType,
  setSearchType,
  query,
  setQuery,
  showOptions,
  setShowOptions,
  filterRef,
  size = "default",
  className = "",
  excludeDepartement = false,
  hasFocus = false,
  setHasFocus = () => {},
  darkMode = false,
  selectedColor = "#22C55E" // Valeur par défaut
}) => {
  const localRef = useRef(null);
  const finalRef = filterRef || localRef;

  const searchOptions = [
    { value: 'nom', label: 'Nom' },
    { value: 'numSerie', label: 'Numéro de série' },
    { value: 'etat', label: 'État' },
    ...(!excludeDepartement ? [{ value: 'departement', label: 'Département' }] : [])
  ];

  const sizes = {
    default: {
      input: "py-2.5 text-sm pl-11 pr-10",
      icon: 18,
      options: "text-sm",
      container: "h-11",
      dropdownIcon: 16
    },
    large: {
      input: "py-3 text-base pl-12 pr-11",
      icon: 20,
      options: "text-base",
      container: "h-12",
      dropdownIcon: 18
    }
  };

  useEffect(() => {
    const handleClickOutside = (event) => {
      if (finalRef.current && !finalRef.current.contains(event.target)) {
        setHasFocus(false);
        setShowOptions(false);
      }
    };

    document.addEventListener('mousedown', handleClickOutside);
    return () => document.removeEventListener('mousedown', handleClickOutside);
  }, [finalRef, setShowOptions, setHasFocus]);

  const currentSize = sizes[size] || sizes.default;
  const currentFilterLabel = searchOptions.find(opt => opt.value === searchType)?.label;

  const handleOptionClick = (optionValue) => {
    setSearchType(optionValue);
    setShowOptions(false);
  };

  const handleClearSearch = () => setQuery("");

  const toggleOptions = (e) => {
    e.stopPropagation();
    setShowOptions(!showOptions);
    setHasFocus(true);
  };

  const handleInputFocus = () => {
    setHasFocus(true);
  };

  return (
    <div className={`relative ${className}`} ref={finalRef}>
      <div className={`relative flex items-center ${currentSize.container} rounded-lg border ${
        darkMode 
          ? "bg-gray-700 border-gray-600" 
          : "bg-white border-gray-200"
      } ${
        hasFocus 
          ? `border-[${selectedColor}] shadow-md` 
          : darkMode 
            ? "hover:border-gray-500" 
            : "hover:border-gray-300"
      } transition-all duration-200`} style={hasFocus ? { borderColor: selectedColor } : {}}>
        
        <div className={`absolute left-3 ${
          darkMode ? "text-gray-400" : "text-gray-400"
        }`}>
          <Search size={currentSize.icon} />
        </div>
        
        <input
          type="text"
          placeholder={`Rechercher par ${currentFilterLabel?.toLowerCase()}`}
          className={`flex-grow ${currentSize.input} bg-transparent outline-none ${
            darkMode ? "placeholder-gray-400 text-gray-100" : "placeholder-gray-400 text-gray-700"
          }`}
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          onFocus={handleInputFocus}
        />
        
        {query && (
          <button
            onClick={handleClearSearch}
            className={`absolute right-10 p-1 transition-colors ${
              darkMode ? "text-gray-400 hover:text-gray-300" : "text-gray-400 hover:text-gray-600"
            }`}
          >
            <X size={currentSize.icon} />
          </button>
        )}
        
        <button
          onClick={toggleOptions}
          className={`absolute right-0 h-full px-3 flex items-center justify-center rounded-r-lg transition-colors ${
            hasFocus 
              ? "text-white"
              : darkMode 
                ? "bg-gray-600 text-gray-300" 
                : "bg-gray-50 text-gray-500"
          }`}
          style={hasFocus ? { backgroundColor: selectedColor } : {}}
        >
          <div className="flex items-center gap-1">
            <Filter size={currentSize.dropdownIcon - 2} />
            <ChevronDown 
              size={currentSize.dropdownIcon} 
              className={`transition-transform ${showOptions ? "rotate-180" : ""}`}
            />
          </div>
        </button>
      </div>
      
      {showOptions && (
        <div className={`absolute right-0 mt-1.5 w-56 rounded-lg shadow-lg z-50 overflow-hidden ${
          darkMode 
            ? "bg-gray-800 border border-gray-700" 
            : "bg-white border border-gray-100"
        }`}>
          <div className="py-1">
            <div className={`px-4 py-2 flex items-center gap-2 border-b ${
              darkMode 
                ? "border-gray-700 text-gray-300" 
                : "border-gray-100 text-gray-600"
            }`}>
              <Filter size={14} className="opacity-70" />
              <span className="text-sm font-medium">Filtrer par :</span>
            </div>
            <div className="py-1">
              {searchOptions.map((option) => (
                <button
                  key={option.value}
                  onClick={(e) => {
                    e.stopPropagation();
                    handleOptionClick(option.value);
                  }}
                  className={`w-full text-left px-4 py-2.5 ${currentSize.options} flex items-center transition-colors ${
                    searchType === option.value 
                      ? "font-medium"
                      : darkMode
                        ? "text-gray-300 hover:bg-gray-700"
                        : "text-gray-700 hover:bg-gray-50"
                  }`}
                  style={
                    searchType === option.value
                      ? { 
                          backgroundColor: darkMode ? `${selectedColor}20` : `${selectedColor}10`,
                          color: selectedColor
                        }
                      : {}
                  }
                >
                  {option.label}
                  {searchType === option.value && (
                    <span className="ml-auto" style={{ color: selectedColor }}>✓</span>
                  )}
                </button>
              ))}
            </div>
          </div>
        </div>
      )}
    </div>
  );
};

export default SearchBar;